...
public class BinanceBybit {
    ...
    public static List<Spread> calculateSpreadBetweenExchanges(List<CoinsResponse> binanceList, List<ListResponse> bybitList) {
        List<Spread> spreadList = new ArrayList<>();
        Map<String, ListResponse> bybitMap = new HashMap<>();
        for (ListResponse bybit : bybitList) {
            if (bybit.getSymbol() != null) {
                String normalized = normalizeSymbol(bybit.getSymbol());
                bybitMap.put(normalized, bybit);
            }
        }
        ...
        for (CoinsResponse binance : binanceList) {
            ...
            ListResponse bybit = bybitMap.get(normalizedBinanceSymbol);
            if (bybit == null) continue;

            try {
                BigDecimal binanceAsk = new BigDecimal(binance.askPrice);
                BigDecimal bybitAsk = new BigDecimal(bybit.getAsk1Price());

                boolean binanceIsShort = binanceAsk.compareTo(bybitAsk) > 0;
                BigDecimal shortPrice = binanceIsShort ? binanceAsk : bybitAsk;
                BigDecimal longPrice = binanceIsShort ? bybitAsk : binanceAsk;
                String shortExchange = binanceIsShort ? "Binance" : "Bybit";
                String longExchange = binanceIsShort ? "Bybit" : "Binance";
    ...
    public static boolean checkSpreadForSymbol(Spread spread) {
        ...
        try {
            ...
            // --- BYBIT ---
            CompletableFuture<BybitMarketTickersResponse> bybitFuture = CompletableFuture.supplyAsync(() ->
                    BybitTradingBot.getCoin(spread)
            );

            // Wait for both to complete
            CompletableFuture.allOf(binanceFuture, bybitFuture).join();

            CoinsResponse binanceCoin = binanceFuture.join();
            BybitMarketTickersResponse bybitCoin = bybitFuture.join();

            BigDecimal binanceAsk = new BigDecimal(binanceCoin.askPrice);
            BigDecimal bybitAsk = new BigDecimal(bybitCoin.result.getList().get(0).getAsk1Price());

            BigDecimal optSpread;
            String direction;
            if (spread.longExchange.equals("Binance")) {
                optSpread = calculateSpread(bybitAsk, binanceAsk);
                direction = String.format("%s Short %s / %s Long %s", spread.shortExchange, bybitAsk, spread.longExchange, binanceAsk);
            } else {
                optSpread = calculateSpread(binanceAsk, bybitAsk);
                direction = String.format("%s Short %s / %s Long %s", spread.shortExchange, binanceAsk, spread.longExchange, bybitAsk);
            }

            BigDecimal thirdSpread = spread.spread.divide(BigDecimal.valueOf(3), SCALE, RoundingMode.HALF_UP);
            LOGGER.info(String.format("Symbol: %s | Direction: %s | Spread: %.4f%%", spread.symbol, direction, optSpread));
            if (optSpread.compareTo(thirdSpread) > 0) {
                needToClose = false;
            }
        } catch (Exception e) {
    ...
    public static boolean checkSpreadForSymbol2(Spread spread) {
        boolean needToClose = true;
        try {
            // --- BINANCE (async) ---
            CompletableFuture<CoinsResponse> binanceFuture = CompletableFuture.supplyAsync(() -> {
            ...
            // (spread.shortExchange / spread.longExchange)
            ...
            if ("Binance".equalsIgnoreCase(spread.shortExchange)) {
                ...
                direction = String.format("Binance Short %s / Bybit Long %s",
                        shortPx.toPlainString(), longPx.toPlainString());
            } else {
                shortPx  = bybitBid;    shortFee = BYBIT_TAKER_FEE;
                longPx   = binanceAsk;  longFee  = BINANCE_TAKER_FEE;
                direction = String.format("Bybit Short %s / Binance Long %s",
                        shortPx.toPlainString(), longPx.toPlainString());
            }

            // Compute spreads
            BigDecimal grossNow = grossSpreadPct(shortPx, longPx);
            BigDecimal netNow   = netSpreadPct(grossNow, shortFee, longFee);   // subtract 0.16%

            // Threshold: compare vs one-third of the ORIGINAL spread.
            // If you added `netSpread` to Spread, prefer that; else fall back to gross.
            //BigDecimal basis = (spread.netSpread != null ? spread.netSpread : spread.spread);
            BigDecimal basis = spread.spread;

            BigDecimal threshold = basis.divide(new BigDecimal("3"), SCALE, RoundingMode.HALF_UP);

            LOGGER.info(String.format(
                    LocalDateTime.now() + " Symbol: %s | Direction: %s | Gross: %.4f%% | Net: %.4f%% | Threshold(1/3): %.4f%%",
                    spread.symbol, direction, grossNow, netNow, threshold));

            // Decision: keep position open if current NET spread is still > 1/3 of original
            // (change logic to > 0 if you want 'close when net<=0')
